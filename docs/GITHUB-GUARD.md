...
本文件只描述 `xpi-research` 的仓库级 GitHub 约束。
...
- `xpi-research` 目前按**阶段一**处理：单人快速迭代优先。
- **只有 `main` 一条长期分支。** 本仓库的常规做法是直接在 `main` 上提交并推送；工作分支只在需要并行试验或临时隔离时才开，用完即删。Agent 不得因为「默认先建分支」的通用规则而在本仓库新建工作分支。
- 当前仓库不启用 `release-please`。
- 如果以后切到阶段二，再把流程改成"分支 + PR + 人工合并"为主，届时同步改掉本节。

## 本仓库约束

- 直推 `main` 是阶段一的常规做法（见「当前阶段」）；`GIT-WORKFLOW.md` §4 的「先建工作分支」在本仓库不适用。
- 合并 PR 仍需用户明确要求并确认后执行；Agent 不自行判断可以合并。
